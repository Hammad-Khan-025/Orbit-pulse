  ...
  return (
    <article>
      ...
      <section className="flex justify-center">
        <div>
        <h2 className="text-2xl md:text-3xl font-bold text-[#047BAF] text-center my-14">
        Our Simple Onboarding Process
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 sm:gap-6 place-items-center items-stretch">
      ...
      <section className="relative">
      <h2 className="text-2xl md:text-3xl font-bold text-[#047BAF] text-center my-18">
      Key Offerings
        </h2>
        <img src={keyOfferingImage} alt="" className="h-72 sm:h-auto object-cover" />
        <div className="bg-[#1D51A2B2] absolute bottom-4 sm:bottom-12 left-0 w-full text-white py-5 sm:py-14 shadow shadow-white px-5 flex flex-col sm:flex-row justify-center gap-3 sm:gap-16 text-xs sm:text-sm">
          {/* Left List */}
      ...
      {/* Questions section */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-x-8 px-5 sm:px-20  pt-20">
      {questions.map((item, index) => (
        <div key={index} className="border-b pb-4">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium">{item.title}</h3>
            <button
              className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center border rounded-full cursor-pointer hover:bg-gray-100"
